...
namespace Bolide
{
    public class KartSuspension : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            GUILayout.BeginArea(new Rect(12, 420, 300, 200), GUI.skin.box);
            GUI.color = Color.white;
            GUILayout.BeginVertical("Box");
            
            GUILayout.Label($"Center Mass Height: {transform.position.y:F2}m");
            GUILayout.Label($"Compression FL: {lastFLcompression:F3}");
            GUILayout.Label($"Compression FR: {lastFRcompression:F3}");
            GUILayout.Label($"Compression RL: {lastRLcompression:F3}");
            GUILayout.Label($"Compression RR: {lastRRcompression:F3}");

            float speed = rb ? rb.linearVelocity.magnitude : 0f;
            GUILayout.Label($"Speed: {speed:F1} m/s ({speed * 3.6f:F1} km/h)");
            GUILayout.Label($"FL Suspension: {lastFLcompression * springStiffness:F0} N");
