...
public class Glider : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        GUI.color = Color.black;
        GUILayout.BeginArea(new Rect(12, 10, 300, 120), GUI.skin.box);
        GUI.color = Color.white;
        GUILayout.Label("Glider");
        GUILayout.Label($"Скорость: {_speedMS:0.0} m/s");
        GUILayout.Label($"Угол атаки: {_alphaRad * Mathf.Rad2Deg:0.0}");
        GUILayout.Label($"Подъёмная сила: {_Lmag:0} N");
        GUILayout.Label($"Сопротивление: {_Dmag:0} N");
        GUILayout.EndArea();
