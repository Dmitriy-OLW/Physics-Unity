...
public class JetEngine : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUI.color = Color.black;
        GUILayout.BeginArea(new Rect(12, 140, 300, 100), GUI.skin.box);
        GUI.color = Color.white;
        GUILayout.Label("JetEngine");
        GUILayout.Label($"РУД: {_throttle01 * 100:0}%");
        GUILayout.Label($"Форсаж: {(_afterBurner ? "ВКЛ" : "ВЫКЛ")}");
        GUILayout.Label($"Тяга: {_lastAppliedThrust:0} N");
        GUILayout.EndArea();
